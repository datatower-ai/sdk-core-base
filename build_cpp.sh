...
target_path="$BASEDIR/output/cpp/"
if [ "$f_benchmark" = true ]; then
  target_path="$BASEDIR/output-benchmark/cpp/"
fi
mkdir -p "$target_path"
...
build_macos() {
  if [ "$f_benchmark" = true ]; then
    ...
    cargo zigbuild --release --package clib --target x86_64-apple-darwin
    cargo zigbuild --release --package clib --target aarch64-apple-darwin
  fi
